...
from rdkit.Chem import Draw, AllChem, Descriptors, Descriptors
...
##        self.caps_lock_timer.start(500)  # Check every 500ms
    ...
    def merge_databases(self, db_files):
        # Create an instance of SQLiteMerger
        merger = SQLiteMerger('merged_molecules.db')

        # Get and display merge information
        merge_info = merger.get_merge_info(db_files)
        self.info_label.setText(merge_info)
...
class DatabaseManager:
    ...
    def fetch_pubchem_data(self, smiles):
        """Fetch data from PubChem using pubchempy and calculate additional properties with RDKit."""
        try:
            ...
            compound = pcp.get_compounds(smiles, namespace='smiles')
            if compound:
                compound = compound[0]  # Get the first compound
            ...
            return data  # Return the default values if there's an error
