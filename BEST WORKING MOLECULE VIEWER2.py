...
##        self.caps_lock_timer.start(500)  # Check every 500ms
    ...
    def populate_database_view(self):
        ...
        for row_idx, molecule in enumerate(molecules):
            ...
            self.db_table.setItem(row_idx, 1, QTableWidgetItem(str(molecule[1])))  # IUPAC Name
            self.db_table.setItem(row_idx, 2, QTableWidgetItem(str(molecule[2])))  # Formula
            self.db_table.setItem(row_idx, 3, QTableWidgetItem(str(molecule[3])))  # Atomic Mass
